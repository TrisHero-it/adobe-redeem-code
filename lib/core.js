...
// pids: danh sách productId cần quét (mặc định tất cả).
async function scanPurl(browser, purl, log = () => {}, pids = COL_ORDER) {
  ...
  for (const pid of pids) {
    ...
    const name = HEADERS[idx] || pid;
    try {
      await page.goto(purl, { waitUntil: 'networkidle2', timeout: 120000 });
    } catch (e) {
      log(`   ${name}: lỗi tải trang (${e.message})`);
      continue;
    }
    ...
    if (link) { byProduct[pid] = link; log(`   ${name}: OK`); }
    else log(`   ${name}: không lấy được`);
    await sleep(400);
  }
  return byProduct; // { productId: link }
}
...
async function runScan(opts = {}) {
  ...
  try {
    for (const { row, pids } of plan) {
      ...
      let byProduct = {};
      try {
        byProduct = await scanPurl(browser, row.purl, log, pids);
      } catch (e) {
        log(`   Lỗi quét: ${e.message}`);
      }
      const gotPids = pids.filter((pid) => byProduct[pid]);
      if (gotPids.length === 0) {
        log(`   -> 0 mã, giữ nguyên hàng.`);
      } else {
        // ghi từng ô của cột được chọn (không đụng cột khác)
        const data = gotPids.map((pid) => ({
          range: `${TAB}!${colLetter(pid)}${row.rowNumber}`,
          values: [[byProduct[pid]]],
        }));
        await sheets.spreadsheets.values.batchUpdate({
          spreadsheetId: SPREADSHEET_ID,
          requestBody: { valueInputOption: 'RAW', data },
        });
        log(`   -> Ghi ${gotPids.length}/${pids.length} mã đã chọn.`);
      }
      done++;
      onProgress({ type: 'row-done', rowNumber: row.rowNumber, got: gotPids.length, done, total: plan.length });
    }
  } finally {
    await browser.close();
  }
